...
- 验证：Provider 接入前全量测试 190 项通过；接入后全量测试 200 项通过；`scripts/check-env.sh` 与 `python3 scripts/publish_policy.py audit` 均通过。
...
- Web 端已通过本地服务启动、`/api/health`、`/api/projects`、本地生成 POST 和媒体预览接口检查；前端 JavaScript 语法检查通过。
...
```text
NEXT: P17-03 本地故事动态分镜试制；公版/热榜研究暂缓
```

任务：

> 用户暂停 V2 后续任务并确定小说改编方向。当前优先用临时试制 IP《镜花缘》在本地走通角色、场景、连续剧情、配音和动态分镜；原公版/热榜匹配与权利证据验收暂缓。P17-01 已验证故事改编到静音粗剪的工作流，P17-02 已把三平台小说榜单接入 P16。

### P17-02 小说候选库接入自动选题池

目标：将平台公开热榜和作品信息整理为可追溯的小说候选快照，接入已有趋势发现/选题池，不抓取或保存整本受版权保护的作品。

验收方向：
- 候选项记录作品、作者、题材、连载状态、平台榜单与采集日期、热度原始口径、来源链接、版本信息和权利状态。
- 各平台热度分开保留；只在同平台、同榜单口径和相近采集时间内分析排名变化，不把阅读人数、月票名次等异构数值直接相加。
- 热度排序与权利准入分开呈现；权利未知的热门作品保留为研究候选，但不能通过小说改编工作流或被标成可制作主 IP。
- 用公开来源记录验证至少三个平台的数据进入候选池后仍保留来源与采集信息，产物标记为人工审核，不自动选主 IP。
- 测试覆盖授权/公版候选、权利未知候选、缺失来源、重复快照和跨平台口径差异。

执行记录（2026-09-16）：
- P15-01：PASS；实现 `scripts/trend_discovery.py`，5 项专项测试通过。
- P16-01：PASS；实现 `scripts/topic_pool.py`，5 项专项测试通过；P15→P16 端到端验证通过。
- 全量验证：184 项测试通过；`scripts/check-env.sh` 和发布边界审计通过。
- Git commit：`4c0d1fa`（P15-01 趋势发现框架、P16-01 自动选题池）。

方向记录（2026-09-16）：
- 用户将后续目标改为“小说 → 国风动漫视频号”；先走通小说来源、改编、分镜、粗剪和人工审核，再通过自动选题池选主 IP。
- V2-03 与 V2-04 暂停；P17-01、P17-02 已通过，当前 NEXT 改为 P17-03。
- 用户随后要求优先在本地做出可行视频，暂不以 IP 可行性作为试制前置条件；当前临时选用《镜花缘》，先完成角色图、配音与最小竖屏视频样片。P17-03 原连续榜单和权利证据研究仍保留为后续验收。
- 本地试制已产出唐小山设定板、竖幅角色图、Tingting 中文配音和 8.32 秒 MP4；素材目录：`projects/jinghua-yuan-local-pilot/`。
- 已继续完成三镜头动态分镜，输出 `tang-xiaoshan-story-animatic.mp4`；本阶段只用本地 FFmpeg 完成运动和剪辑，真实角色逐帧动作仍待专用视频生成 Provider。
- P17-01 代码 Git commit：`6a2fce6`（小说改编流程试点）。
- P17-02 代码 Git commit：`233038a`（小说候选池接入 P16）。
