...
func _build_background() -> void:
    ...
    var radius := Vector2(285.0, 410.0)
...
func _build_character() -> bool:
    ...
    var scale_value: float = min(620.0 / canvas_size.x, 1120.0 / canvas_size.y)
    ...
    while not pending.is_empty() and guard < 64:
        ...
        for item_variant in pending.duplicate():
            ...
            var pivot := Vector2(float(pivot_data.get("x", 0)), float(pivot_data.get("y", 0)))
            var parent_node: Node2D = character_root if parent_name.is_empty() else layer_nodes[parent_name]
            var parent_pivot := Vector2.ZERO
...
func _build_contact_shadow() -> void:
    ...
    var center := Vector2(width * 0.5, height * 0.88)
    var radius := Vector2(width * 0.22, height * 0.025)
    for index in range(segments):
        var angle: float = TAU * float(index) / float(segments)
        points.append(center + Vector2(cos(angle) * radius.x, sin(angle) * radius.y))
...
func _process(delta: float) -> void:
    ...
    if layer_nodes.has("torso"):
        var torso: Node2D = layer_nodes["torso"]
        torso.scale = Vector2(1.0 + breath * 0.004, 1.0 + breath * 0.009)
        torso.rotation = deg_to_rad(wave * 0.45)

    if layer_nodes.has("head"):
        var head: Node2D = layer_nodes["head"]
        head.rotation = deg_to_rad(wave * 1.2)
        head.position = base_node_positions["head"] + Vector2(wave * 1.5, -breath * 1.2)

    if layer_nodes.has("upper_arm_l"):
        layer_nodes["upper_arm_l"].rotation = deg_to_rad(-18.0 * raise_amount + wave * 1.4)
    if layer_nodes.has("forearm_l"):
        layer_nodes["forearm_l"].rotation = deg_to_rad(-24.0 * raise_amount + sin(progress * TAU - 0.45) * 1.8)
    if layer_nodes.has("hand_l"):
        layer_nodes["hand_l"].rotation = deg_to_rad(9.0 * raise_amount + sin(progress * TAU - 0.8) * 1.5)

    if layer_nodes.has("upper_arm_r"):
        layer_nodes["upper_arm_r"].rotation = deg_to_rad(wave * -2.0)
    if layer_nodes.has("forearm_r"):
        layer_nodes["forearm_r"].rotation = deg_to_rad(sin(progress * TAU - 0.35) * -2.6)
    if layer_nodes.has("hand_r"):
        layer_nodes["hand_r"].rotation = deg_to_rad(sin(progress * TAU - 0.6) * -1.8)

    for name_variant in layer_sprites.keys():
        var name: String = str(name_variant)
        var sprite: Sprite2D = layer_sprites[name]
        ...
        sprite.position = base_sprite_positions[name] + Vector2(
