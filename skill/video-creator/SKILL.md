...
- Research 阶段先读取 `skill/video-creator/research/SKILL.md`，按 `source_mode` 调查并整理带引用的 brief，然后运行 `./video-creator research <project-id> --input-file <JSON>` 生成三个研究产物。CLI 校验来源并推进至 `RESEARCHED`；未配置 Search Provider 时由 Codex 搜索能力完成采集，不得假装本地 CLI 已搜索网页。
