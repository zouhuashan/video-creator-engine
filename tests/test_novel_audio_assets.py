...
from scripts.novel_audio_assets import NovelAudioAssetError, build_audio_assets, validate_audio_assets, write_audio_assets
...
class NovelAudioAssetTests(unittest.TestCase):
    def make_project(self, root: Path) -> Path:
        project = root / "project-test"; write_project(project, build_project("project-test", "TST", "测试故事")); write_catalog(project, build_catalog("project-test", "IP-TST", "测试故事")); write_bible(project, build_bible(project)); write_plan(project, build_plan(project)); write_episode_planning(project, build_episode_planning(project)); write_script_package(project, build_script_package(project)); write_visual_bible(project, build_visual_bible(project)); NovelAnimeRepository(project).initialize(); write_environment_assets(project, build_environment_assets(project)); write_asset_review(project, build_asset_review(project)); write_shot_breakdown(project, build_shot_breakdown(project)); write_storyboard(project, build_storyboard(project)); write_animatic(project, build_animatic(project)); return project
    def test_empty_audio_plan_has_mix_defaults(self):
        with tempfile.TemporaryDirectory() as directory:
            project = self.make_project(Path(directory)); package = build_audio_assets(project); output = write_audio_assets(project, package); self.assertTrue(output.is_file())
        self.assertEqual(package["mix_profile"]["sample_rate"], 48000); self.assertEqual(package["tracks"], [])
    def test_animatic_revision_drift_blocks_audio(self):
        with tempfile.TemporaryDirectory() as directory:
            project = self.make_project(Path(directory)); package = build_audio_assets(project); package["animatic_revision"] += 1
            with self.assertRaisesRegex(NovelAudioAssetError, "upstream revision"): validate_audio_assets(project, package)
