...
from scripts.novel_dynamic_shots import NovelDynamicShotError, build_dynamic_shots, validate_dynamic_shots, write_dynamic_shots
...
class NovelDynamicShotTests(unittest.TestCase):
    def make_project(self, root: Path) -> Path:
        project = root / "project-test"; write_project(project, build_project("project-test", "TST", "测试故事")); write_catalog(project, build_catalog("project-test", "IP-TST", "测试故事")); write_bible(project, build_bible(project)); write_plan(project, build_plan(project)); write_episode_planning(project, build_episode_planning(project)); write_script_package(project, build_script_package(project)); write_visual_bible(project, build_visual_bible(project)); NovelAnimeRepository(project).initialize(); write_environment_assets(project, build_environment_assets(project)); write_asset_review(project, build_asset_review(project)); write_shot_breakdown(project, build_shot_breakdown(project)); write_storyboard(project, build_storyboard(project)); write_animatic(project, build_animatic(project)); write_voice_profiles(project, build_voice_profiles(project)); write_audio_assets(project, build_audio_assets(project)); write_audio_mix(project, build_audio_mix(project)); return project
    def test_dynamic_plan_starts_with_zero_local_cost(self):
        with tempfile.TemporaryDirectory() as directory:
            project = self.make_project(Path(directory)); package = build_dynamic_shots(project); output = write_dynamic_shots(project, package); self.assertTrue(output.is_file())
        self.assertEqual(package["budget"]["limit"], 0); self.assertEqual(package["shot_routes"], [])
    def test_billable_job_requires_confirmation_and_upload_authorization(self):
        with tempfile.TemporaryDirectory() as directory:
            project = self.make_project(Path(directory)); package = build_dynamic_shots(project); package["jobs"] = [{"id": "DYNJOB-TST-001", "shot_id": "SHOT-MISSING", "provider": "runway", "billable": True, "confirm_billable": False, "upload_authorized": False, "status": "QUEUED", "retry_count": 0, "max_retries": 1, "output_path": None}]
            with self.assertRaisesRegex(NovelDynamicShotError, "unknown shot"):
                validate_dynamic_shots(project, package)
