...
class WebServerTests(unittest.TestCase):
    ...
    def test_p35_gpt_keyframe_web_exposes_codex_batch_manual_fallback_and_local_interpolation(self):
        ...
        self.assertIn('id="gptCodexLogsRefresh"', index)
        self.assertIn('id="gptCodexLogsPanel"', index)
        ...
        self.assertIn("/graybox/gpt-keyframes/codex-batch/logs", app)
        self.assertIn("loadCodexBatchLogs", app)
        self.assertIn("renderCodexBatchLogs", app)
        ...
        self.assertIn("codex-batch/logs", server)
    ...
    def test_image_studio_explains_cinematic_3d_default_and_blocks_local_preview(self):
        index = (web_server.WEB_ROOT / "index.html").read_text(encoding="utf-8")
        app = (web_server.WEB_ROOT / "app.js").read_text(encoding="utf-8")
        self.assertIn("参考视频·电影级 3D 国漫", index)
        self.assertIn("本地 Animagine 只保留给概念预览", index)
        self.assertIn("参考视频 3D 最终视觉", index)
        ...
        self.assertIn("BLOCKED · Final requires OpenAI", app)
        self.assertIn("生成最终 3D LookDev", app)
    ...
    def test_image_style_presets_default_to_cinematic_3d_and_keep_illustration_lora_separate(self):
        ...
        self.assertEqual(final_3d["layout_mode"], "SINGLE_LOOKDEV_HERO")
        self.assertTrue(final_3d["final_provider_required"])
        self.assertEqual(final_3d["lora_id"], "")
        self.assertIn("semi-realistic 3D Chinese donghua", final_3d["positive_prompt_prefix"])
        self.assertIn("warm sunset or lantern backlight", final_3d["positive_prompt_prefix"])
        self.assertIn("multi-panel layout", final_3d["negative_prompt"])
    ...
    def test_web_exposes_p32_graybox_workflow_without_terminal_steps(self):
        ...
        self.assertIn('id="grayboxInstallSmokePack"', index)
        self.assertIn("/graybox/references/install-smoke-pack", app)
        self.assertIn("installGrayboxSmokePack", app)
        ...
        self.assertIn("PROCESS ALIVE", app)
        self.assertIn("STALLED?", app)
        self.assertIn("syncGrayboxPolling", app)
        self.assertIn("/graybox/spec", app)
        self.assertIn("READY · REVIEW", app)
        self.assertIn("✓ 镜头方案已就绪", app)
        self.assertIn("镜头方案处理中…", app)
        self.assertIn("下一步点击“② 生成 Blender 白模”", app)
