...
from scripts.novel_voice_profiles import NovelVoiceProfileError, build_voice_profiles, validate_voice_profiles, write_voice_profiles
from scripts.novel_visual_bible import build_visual_bible, write_visual_bible


class NovelVoiceProfileTests(unittest.TestCase):
    ...
    def test_empty_story_has_valid_voice_plan(self):
        with tempfile.TemporaryDirectory() as directory:
            project = self.make_project(Path(directory)); package = build_voice_profiles(project); output = write_voice_profiles(project, package); self.assertTrue(output.is_file())
        self.assertEqual(package["profiles"], []); self.assertEqual(package["line_assignments"], [])

    def test_script_revision_drift_blocks_voice_plan(self):
        with tempfile.TemporaryDirectory() as directory:
            project = self.make_project(Path(directory)); package = build_voice_profiles(project); package["script_revision"] += 1
            with self.assertRaisesRegex(NovelVoiceProfileError, "upstream revision"):
                validate_voice_profiles(project, package)
