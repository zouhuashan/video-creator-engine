...
class GrayboxReferenceBindingTests(unittest.TestCase):
    ...
    def test_packaged_smoke_reference_pack_installs_and_binds_both_images(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(result["source_character"], "assets/p32/GB-SHOT-001/character-reference.webp")
            self.assertEqual(result["source_scene"], "assets/p32/GB-SHOT-001/scene-reference.webp")
            self.assertTrue(refs._image_valid(refs.SMOKE_REFERENCE_CHARACTER))
            self.assertTrue(refs._image_valid(refs.SMOKE_REFERENCE_SCENE))
