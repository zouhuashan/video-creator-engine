...
def load_config(path: Path = CONFIG_PATH) -> dict[str, Any]:
    ...
    if config.get("missing_asset_recovery") != ["search", "generate", "replace_with_infographic", "request_user_material"]:
        raise AssetRoutingError("missing asset recovery order is invalid")
    return config
...
def recover_missing_asset(available_actions: list[str], config: dict[str, Any] | None = None) -> dict[str, str]:
    active = config or load_config()
    allowed = active["missing_asset_recovery"]
    if not isinstance(available_actions, list) or any(action not in allowed for action in available_actions):
        raise AssetRoutingError("available_actions contains an unsupported recovery action")
    action = next((candidate for candidate in allowed if candidate in available_actions), "request_user_material")
    return {"status": "MISSING_ASSET", "next_action": action}
