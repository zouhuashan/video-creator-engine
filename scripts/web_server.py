...
def _graybox_web_status(project: Path) -> dict[str, object]:
    ...
    return {
        ...
        "render": {
            ...
            "current_frame": int(render.get("current_frame") or 0),
            "total_frames": int(render.get("total_frames") or 0),
            "progress_percent": float(render.get("progress_percent") or 0.0),
            "heartbeat_seconds_ago": render.get("heartbeat_seconds_ago"),
            "log_updated_seconds_ago": render.get("log_updated_seconds_ago"),
            "progress_source": str(render.get("progress_source") or ""),
...
class VideoCreatorHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if match:
            try:
                project = _safe_project(match.group(1))
                return self._json(codex_keyframe_batch_logs(project))
